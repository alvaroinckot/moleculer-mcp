...
import { BridgeOptions, CustomTool } from "../config/BridgeOptions";
...
export class ServiceCatalogue {
  ...
  private buildCatalogue(): void {
    ...
    console.log(`Found ${actions.length} total actions, ${allowedActions.length} allowed by settings`);
  ...
  private filterAllowedActions(actions: ActionInfo[]): ActionInfo[] {
    return actions.filter(action => 
      this.isActionAllowed(action.name, this.options.allow)
    );
  }
  ...
  private createToolConfig(
    ...
    if (paramOverrides && zodSchema instanceof z.ZodObject) {
      ...
      for (const [key, value] of Object.entries(shape)) {
        if (paramOverrides.hasOwnProperty(key) && value && typeof value === 'object' && 'optional' in value) {
    ...
    this.toolConfigs[toolName] = {
      ...
      handler: async (args: any) => {
        try {
          ...
          console.log(`MCP tool '${toolName}' calling action '${originalActionName}' with args:`, finalArgs);
