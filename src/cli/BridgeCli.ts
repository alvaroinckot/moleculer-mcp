...
export class BridgeCli {
  ...
  private setupCommands(): void {
    this.program.name("moleculer-mcp").description("Moleculer MCP Bridge CLI").version("1.0.0");
    ...
    this.program
      ...
      .option(
        "-m, --moleculer-config <path>",
        "Moleculer configuration file path (moleculer.config.js)"
      )
  ...
  private async handleStart(
    configPath?: string,
    options?: { moleculerConfig?: string }
  ): Promise<void> {
    try {
  ...
  private async handleListActions(options: {
    config?: string;
    moleculerConfig?: string;
  }): Promise<void> {
    ...
    try {
      ...
      actions.forEach((action) => {
        const isExposed = Object.values(tools).length > 0; // Simplified check
